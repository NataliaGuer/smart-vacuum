...
def not_in(state, explored_set):
    if len(explored_set) == 0:
        return True
    for explored in explored_set:
        if (explored.grid == state.grid and 
            explored.pos_x == state.pos_x and
            explored.pos_y == state.pos_y):
            return False
    return True
...
def breadth_first_graph_search(problem):
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            if not_in(child.state, explored) and child not in frontier:
...
def best_first_graph_search(problem, f, display=False):
    ...
    while frontier:
        node = frontier.pop()
        # print(f'node: {node.state.pos_x}, {node.state.pos_y}')
        ...
        explored.add(node.state)
        for child in node.expand(problem):
            # print(f'child: {child.state.pos_x}, {child.state.pos_y}, {child.action}')
            # print(child.state.grid)
            if not_in(child.state, explored) and child not in frontier:
                frontier.append(child)
